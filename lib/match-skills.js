...
class MatchSkills {
  ...
  static matchAll(a1, a2, returnType){
    ...
    (Array.isArray(a1) ? a1 : []).forEach(s1 => {
      (Array.isArray(a2) ? a2 : []).forEach(s2 => {
        if (typeof s1 !== "undefined" && typeof s1.id !== "undefined" &&
          ...
          const s1trim = s1.trim().toLowerCase();
          if (s1trim === s2.trim().toLowerCase()) {
            map.set(s1trim, s1);
            score += 1;
          }
        }
  ...
  static matchScore(a1, a2){
    return Promise.resolve(
      ...
      (Array.isArray(a2) ? a2 : []).reduce((s, s2) => ((typeof s1 === 'string' || typeof s2 === 'string') ? (`${s1}` === `${s2}`) : (s1.id === s2.id)) ? s + 1 : s
      , 0), 0))
  }
}
